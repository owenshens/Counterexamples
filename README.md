...
what survives. This repository holds the results: 158 short papers, each with the code that
checks it.

**[OPEN_RESULTS.md](OPEN_RESULTS.md)** — the list: **99 papers refuting a published
conjecture or question**, **18 exhibiting an object whose existence was open**, and
**41 proving a conjecture or recording a note**. Two statements each carry two independent
papers, which exhibit different objects and are listed separately.

| directory | n | contents |
|---|---|---|
| [`counterexamples/`](counterexamples/) | 99 | a published conjecture or question is refuted |
| [`constructions/`](constructions/) | 18 | an object whose existence was open is exhibited |
| [`notes/`](notes/) | 41 | conjectures proved, expository notes, and re-proofs of results due to others |
...
prior art before publication. A paper was withheld if the statement it settles turned out not
to be the statement its source posed, if its program did not establish its headline, or if the
result was already in the literature.

## The verification programs

**123 of the 158 papers ship a program and a transcript.** The remaining 35 need none: their
...
the 123 programs the recorded runs report **6,994 checks, all passing**.
