...
Proposition~\ref{prop:census}. The census source code, the \texttt{plantri}
source used and its SHA-256 hash, the compilation and execution commands, and
the logs are not part of this deposit; none of them is needed for the argument
above. What accompanies this note is a self-contained verification script for
the two graphs recorded below. It rebuilds $T$ from the recipe above, certifies
that $T$ and $W$ are planar by exhibiting a sphere face set, confirms by
exhaustive search over six-subsets that neither contains an induced $P_6$, and
prints an induced $P_6$ for each of the $34$ facial insertions of $T$. The
census sweep itself is not repeated there.
