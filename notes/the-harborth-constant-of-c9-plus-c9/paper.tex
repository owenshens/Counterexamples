...
node count is specific to an implementation. That program accompanies this
paper, together with the transcript of a run of it made under a bounded time
budget: in the transcript the counts listed above are re-derived in full,
while the census over the $831$ feasible prefixes is carried only as far as
the budget allows, the uncovered remainder being reported explicitly rather
than passed over; the census is completed by running the program with no time
budget. Nothing above depends on either file. The re-run is a second
