...
parameters remaining positive. Three features of \cite{KS} support this
reading. First, \eqref{eq:aggregate} is automatic under the strict
inequalities $b_i>a_i$ of Theorem~3, so on a reading that retains them it
would be an idle clause. Second, Conjecture~2
...
These three features do not exclude one intermediate reading, and we do not
claim that they do: one may take Conjecture~1 to assert \eqref{eq:bound}
under $b_i\ge a_i>0$ together with \eqref{eq:aggregate}, a hypothesis under
which the aggregate clause is not idle, since it then excludes only $A=B$,
and which the non-strict form of Luke's result recorded in \cite{KS} makes
salient. Our example does not bear on that intermediate reading: it has
$b_1=1$ and $a_1=2$, so even $b_1\ge a_1$ fails, and indeed no violation of
\eqref{eq:bound} for $x>0$ can occur when $b_i\ge a_i>0$, precisely because
of the bound credited to Luke. What Theorem~\ref{thm:counterexample}
refutes is the reading stated above, on which \eqref{eq:aggregate} replaces
the componentwise inequalities altogether; on the intermediate reading the
