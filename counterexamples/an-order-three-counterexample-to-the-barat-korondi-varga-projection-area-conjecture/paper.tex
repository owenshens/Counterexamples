...
control against systematic undershoot; and a mutation test, carried out
separately while the program was being written and not part of the transcript
it produces, reported failure under each of a suite of deliberate corruptions
of the printed data and of the program's own geometry, with one exception that
is provably immaterial on this input, namely interchanging the first two rows
of the table, whose printed neighbourhoods both lie inside $B$. The program
and its transcript accompany this note; every quantity above is in any case
recomputable from the data printed here together with the definitions of the
