...
# -*- coding: utf-8 -*-
"""
verify_selfadhesivity_counterexample.py  --  verifier for paper stem 14
(a five-variable self-adhesivity counterexample: a supermodular integer set
function f on N = {0,1,2,3,4} whose induced structural semi-graphoid M is
self-adhesive-at-every-overlap yet is NOT induced by a self-adhesive
supermodular function -- Lemmas 1/3/4 and Remark 2 of the paper).
...
=====================  WHAT IS TAKEN FROM THE PAPER  =====================
  ...
  W3  the linear certificate of Lemma 4: 10 LHS terms with
  ...
  EXP expected-value constants used ONLY as comparison targets, never
      ...
      (|A_L|,|sg(A_L)|) of the Lemma-3 table; rank 19; ambient dim 26;
      face dim 7; Delta_f(04|3) = 1; |M| = 20.
  AUD six (|A_L|,|sg(A_L)|) pairs for overlaps the paper does NOT
      tabulate, reported by an independent audit implementation.
      NOT paper data and NOT part of any pass/fail decision: they are
      printed as an implementation-vs-implementation cross-check only.
...
=====================  WHAT THE PROGRAM DERIVES  ========================
  ...
  * |F| = 54 for the Lemma-4 forced set and F == A_{012} built by the
    independent Lemma-3 code path
  ...
  * that the contradiction target 04|3 REALLY IS a term of the
    ...
    -c*Delta_{04|3} -- the link Lemma 4 needs and that nothing in the
...
# ------------- W4/EXP: the Lemma-3 table  L -> (|A_L|, |sg(A_L)|) -----------
LEMMA3_TABLE = {
...
# by an INDEPENDENT audit implementation (spec notes), not by the paper.  Used
# only as an informational cross-check between two implementations: a mismatch
# is printed loudly but does not by itself impeach the paper.
AUDIT_UNTABULATED = {
    "": (6440, 7680),
...
# ===========================================================================
#  SECTION 1  --  Lemma 1: f is supermodular and its zero set is M
# ===========================================================================

def section_lemma1():
    print("--- SECTION 1: Lemma 1 (f supermodular, M_f = M) " + "-" * 22)
...
# ===========================================================================
#  SECTION 2  --  Remark 2: rank 19, ambient dim 26, face dim 7, interior
# ===========================================================================

def section_remark2(diffs, M):
    print("--- SECTION 2: Remark 2 (rank / face dimension) " + "-" * 24)
...
# ===========================================================================
#  SECTION 3  --  Lemma 3: A_L, sg(A_L) and the N-marginal, for ALL 32 L
# ===========================================================================

def analyse_overlap(M, L):
    """Derive every Lemma-3 quantity for one overlap L.  Returns a dict."""
    d = adhesion_set(M, L)
...
def section_lemma3(M):
    print("--- SECTION 3: Lemma 3 (all 32 overlaps L) " + "-" * 29)
    ...
    for key in sorted(LEMMA3_TABLE, key=lambda k: (len(k), k)):
        d = data[key]
        exp_A, exp_sg = LEMMA3_TABLE[key]
    ...
    always_true = all(not (set(M) - data[k]["marginal"]) for k in LEMMA3_TABLE)
    ...
    note("CROSS-CHECK against an independent implementation on %d untabulated "
         "rows (these are NOT paper values): %d agree, %d differ%s"
    ...
    n_untab = len(every) - len(LEMMA3_TABLE)
...
# ===========================================================================
#  SECTION 4  --  Lemma 4: the forced set F and the linear certificate
# ===========================================================================

def section_lemma4(f, M, lemma3_data):
    print("--- SECTION 4: Lemma 4 (forced set F, certificate) " + "-" * 21)
    ...
    record("forced_set_F_construction",
           ...
           "Lemma-3 code path: %s; G_{012} == {0..6}: %s; all of F lives on "
...
TARGET_TEXT = "04|3"        # the statement Lemma 4 forces to be zero
...
def section_target_is_on_the_lhs(M, F):
    ...
    Lemma 4 needs and neither of which was asserted anywhere, are:
...
def main():
    print("=" * 78)
    print("VERIFIER for paper stem 14 -- five-variable self-adhesivity counterexample")
    ...
    print("  the Lemma-3 table, rank 19, dim 26, face 7, Delta_f(04|3)=1) are used")
    ...
    f, diffs, M = section_lemma1()
    print("")
    rank, _face = section_remark2(diffs, M)
    print("")
    lemma3 = section_lemma3(M)
    print("")
    section_lemma4(f, M, lemma3)
    print("")
