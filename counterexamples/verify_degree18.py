...
# -*- coding: utf-8 -*-
"""
verify_degree18.py -- exact-integer verification of the degree-18 counterexample to
...
=====================================================================================
  P1. The witness y: 30 integer values indexed by the even partitions of 18
      (the table in eq:witness).  Transcribed literally below as WITNESS_RAW.
...
=====================================================================================
  ...
  D13. BEYOND THE PAPER (derived, never a paper claim): the size of the full
       second-order system on Lambda^{ev}_18 (comes out 7025), its minimum slack
       (-2), and the fact that exactly one inequality is violated; plus the
       structural embedding T^(2) subset T^(1), which is the only check tying the
       two definitions to each other rather than to one worked example.
...
# ------------------------------------------------------------------------------
# FROM THE PAPER (P1): the witness, transcribed literally from eq:witness.
...
# NOT FROM THE PAPER.  27.tex states only "15 nonempty blocks ... containing 131
# terms in total"; it prints NO per-block table.  These 15 numbers were RECORDED
...
# DERIVED (D13), reported as derived, never as a paper claim.  Like
# RECORDED_BLOCK_SIZES this is a REGRESSION PIN recorded from a previous run of
# this code -- useful, but not an independent derivation of anything.
...
# ==============================================================================
# INDEPENDENT CROSS-DERIVATION of every published count (added by adversarial
# review).  Everything below COUNTS by integer dynamic programming and shares no
# code with even_partitions() or alpha_tuples(), which ENUMERATE.  Without this,
# all five published counts flowed from one enumeration path and the two printed
# "cross-checks" were algebraic identities on that same path -- they could not
# disagree with it.  These can.
# ==============================================================================
...
# ==============================================================================
# DERIVED D13 (beyond the paper): the FULL second-order system on Lambda^{ev}_18.
# ==============================================================================
...
class Report(object):
    ...
    def verdict(self):
        ...
        if not bad:
            ...
            print("  inequalities, the five counts, and the second-order violation).")
            print("  The step 'y in T^(1) AND those 1056 inequalities => y in")
            print("  trop(BSigma_18^*)' is Lemma lem:test of the paper, proved by")
            print("  citation into ABDR and NOT verified here.  A green run does not")
            print("  establish the paper's theorem on its own.")
            return 0
        for name in bad:
...
def check_C0(rep):
    ...
    # 30 is NOT a number 27.tex prints -- the paper prints a 30-row table.  So it
...
def check_C13(rep, coords, y, l1, l2, mu, fo):
    """C13 global consistency of the violation.  DERIVED -- NOT a paper claim.

    Also carries the ONE structural check that links the T^(1) and T^(2)
    definitions to each other:  T^(2) subset T^(1).  Before this was added, the
    DIRECTION of superdominance and the meaning of mu^{o2} (multiplicity doubled,
    part values unchanged) were each pinned by exactly one worked example -- the
    ...
    """
    print("--- C13: the FULL second-order system on Lambda^{ev}_18 (DERIVED, not "
          "from the paper) ---")
    ...
    rep.note("size %d is a REGRESSION PIN recorded from a previous run of this code "
             "-- never a paper claim, and not an independent derivation"
             % len(system))
    rep.check("C13 global consistency of the violation, and T^(2) subset T^(1) "
              "(derived, not from the paper)",
...
def main():
    print("=" * 84)
    print("verify_degree18.py -- degree-18 counterexample to ABDR Conjecture 5.23")
    ...
    print("  lists, counts and slacks -- plus the full T^(2) system (7025), which")
    print("  is a derived fact and NOT a paper claim.")
    ...
    print("NOT VERIFIED HERE: the membership Lemma (see the TRUST BOUNDARY at C12).")
    print("=" * 84)
    rep = Report()

    # NOTE: every ok_Cn below is the boolean the corresponding check ACTUALLY
    # asserted and printed, not a weaker predicate re-derived here.  Previously
    # main recomputed them, dropping the all-int conjuncts and, for C10, dropping
    # the coordinate/shape/prefix-vector comparisons -- so a C10 FAIL on the
    # paper's printed prefix sums would not have propagated into C12.
    ...
    check_C13(rep, coords, y, l1, l2, mu, fo)
