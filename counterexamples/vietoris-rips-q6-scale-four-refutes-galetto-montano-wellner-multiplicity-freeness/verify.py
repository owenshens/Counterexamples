...
# -*- coding: utf-8 -*-
"""
verify.py -- ancillary verification for the Vietoris-Rips complex
...
---------------------------------------------------------------------------
  ...
  P3. LEMMA 2 of the paper, ASSUMED and NOT reproved:
      ...
      homology.
  P4. The recurrence (3) J(F) = J(F - v) - J(F - N[v]) with
  ...
  P7. Equation (1) of the paper, cited from GMW Appendix C.3:
...
---------------------------------------------------------------------------
  ...
  D3. Every class size from equation (2), the total 46080, and the
  ...
  D10. A FULL CENSUS of all 6! * 2^6 = 46080 elements of G (C12): each
      ...
      equation (2) is checked CLASS BY CLASS rather than only in total,
...
---------------------------------------------------------------------------
  ...
  PAPER_H7_DIM = 239                equation (1); C9 only
  PAPER_H15_DIM = 14                equation (1); C9 only
  PAPER_HOMOLOGY_DEGREES = (7, 15)  equation (1); C9 (signs), C10 (prose)
...
NOT DERIVABLE HERE (stated as limitations, not verified):
  ...
  X^{6,4} vanishes outside degrees {7, 15}; Lemma 2 itself;
...
# P7 -- equation (1) from GMW Appendix C.3, cited by the paper.
PAPER_H7_DIM = 239
PAPER_H15_DIM = 14
...
def class_size(alpha, beta):
    """Equation (2) of the paper, with an EXACT-division assertion (P5)."""
...
def apply_g(a, perm, x):
    ...
    The other convention gives a conjugate of g^{-1}, which has the same
    orbits up to Hamming isometry, so every derived quantity is
    unchanged."""
    y = 0
    for j in range(N):
        if (x >> j) & 1:
...
# ===========================================================================
# D6 -- J(F) = I(F;-1) via recurrence (3), memoised, component-multiplicative
# ===========================================================================
...
def J_value(mask, adj, memo):
    """J(F[mask]) = I(F[mask]; -1) by recurrence (3) (P4).
...
def J_value_alt(mask, adj, memo, budget):
    ...
    Same recurrence (3), same exact integer arithmetic.  Raises
...
def run_C2(classes):
    """C2: class sizes positive integers from equation (2), summing to |G|."""
...
# ===========================================================================
# C9 -- identity class cross-check against equation (1)
# ===========================================================================
def run_C9(records):
    section("C9  identity-class cross-check against equation (1)")
    idt = "111111|-"
    ...
    info("paper's equation (1), cited from GMW C.3: dim H_7 = %d, "
         "dim H_15 = %d, sum = %d"
    ...
    # chi~(K) = sum_i (-1)^i dim H~_i, evaluated on equation (1) using the
    ...
    info("chi~(K) from equation (1) as sum_i (-1)^i dim H~_i over degrees "
         "%s = %s = %d"
    ...
    ok &= check("C9 chi~(K) from the derived Psi equals sum_i (-1)^i dim H~_i "
                "from equation (1), and both cited degrees are odd so the "
                "signs are +1 in Psi",
...
def run_C12(records, sample_per_type=3):
    ...
    previously unasserted anywhere in this file:
      ...
      (a) that the equation (2) VALUE equals the ACTUAL number of elements
          ...
          compensating directions -- so a paper error in equation (2) of
    ...
    for t in size_bad[:5]:
        info("  |C| CENSUS MISMATCH %-12s census %s, equation (2) %s"
             % (t, census.get(t), derived_size.get(t)))
    info("types whose census count equals the equation (2) value: %d of %d"
    ...
    ok &= check("C12 equation (2) is correct CLASS BY CLASS, not merely in "
...
def main():
    ...
    print("verify.py -- K = X^{6,4} = VR(Q_6;4), G = H_6 = "
          "F_2^6 : S_6")
    print("Checking: Psi(g) = I(F_g;-1) on all conjugacy classes; "
          "sum |C| = 46080; sum |C| Psi = 138240; average 3.")
    print("FROM THE PAPER: n=6, r=4; the group and its action; Lemma 2 "
          "(assumed, proved in the paper);")
    print("                the recurrence (3) and the class-size formula (2); "
          "the representative recipe;")
    print("                equation (1): 239 + 14 from GMW C.3; character "
          "averaging; and the")
...
if __name__ == "__main__":
    try:
        ...
        # so the transcript still ends in a verdict line.
