...
all-$6$-isolating sets, even without an independence requirement.  They are
exactly the counterexamples of minimum order among trees of order different
from $6$.  On the other hand, every tree has a $6^{*}$-coloring whose color
classes are all-$6$-isolating, so six is the optimal universal guarantee.
\end{abstract}
...
for $k\geq 7$, and left the case $k=6$ open
\cite[Theorem~7 and the paragraph following Lemma~8]{BFG}.
...
pairwise disjoint nonempty all-$6$-isolating sets.  Up to isomorphism, these
are exactly the counterexamples of minimum order among trees of order
not equal to $6$.  In particular, neither spider has an $\ell^{*}$-coloring
